...
<body>
 	<form id="save_form" action="${ctx}/wxapi/reply/news/save" method="post">
    	...
    	<div id="content">
	    	<div class="flag"> 
		    	...
		    	<div class="rightBtn">
		    		<c:choose>
			    		<c:when test="${empty(mode) && empty(type)}">
			    			<a href="${ctx}/wxapi/reply/news/init/${replyId}">返回图文列表</a>
						</c:when>
						<c:otherwise>
							<a href="${ctx}/wxapi/reply/init/${mode}?type=${type}">返回图文列表</a>
						</c:otherwise>
		    		</c:choose>
		    	</div>		    		
			</div>
			...
			<div class="pBtn">
				<c:choose>
					<c:when test="${empty(mode) && empty(type)}">
						<a class="save orangeBg" href="javascript:htmSave('${ctx}/wxapi/reply/news/init/${replyId}');">保&nbsp;&nbsp;存</a>
					</c:when>
					<c:otherwise>
						<a class="save orangeBg" href="javascript:htmSave('${ctx}/wxapi/reply/init/${mode}?type=${type}');">保&nbsp;&nbsp;存</a>
					</c:otherwise>
				</c:choose>
