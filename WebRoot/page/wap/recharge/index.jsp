		...
		$.pop.safe({
			...
			call : function(code, pswd) {
				...
				$.ajax({
					...
					url : '${ctx}/wap/recharge/submitRecharge?openid=${openid}&&userId=${userId}&cardId=${card.id}&mobile=${mobile}&money='+$("#money").val(),
					data : {
						payPassword : pswd
					},
					error : function(e) {
					...
					success : function(response) {
						...
						window.setTimeout(function() {
							...
							window.setTimeout(function() {
								if (response.result == 1) {
									var url = '${ctx}/wap/recharge/success?openid=${openid}&money='+$("#money").val();
									location.href = url;
								} else {
									iTips(response, code);
								}
  ...
  <c:if test="${need_head}">
   ...
   <div class="bankcash">
    <ul class="bankcashlist">
      <li>
         <p>金额</p>
         <input class="inputmoney" id="money" type="text" placeholder="请输入金额"/>
      </li>
    </ul>
   </div>
   <p class="pbalance">当前账户余额<span>${balance.surplusBalance}</span>元</p>
   <a class="confirm" href="javascript:submit();">确认充值</a>
   <a class="bankrules">《银行卡支付限额规定》</a>
</body>
