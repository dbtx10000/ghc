...
<script type="text/javascript">
	...
	function InitEvent(canvasContainer, cxt) {
        ...
        canvasContainer.addEventListener("touchend", function (e) {
			...
			if (touchsPassword == '') {
				touchsPassword = password.join("");
				$("#error").html('请重复您的手势密码');
			} else {
				if (touchsPassword != password.join("")) {
					$("#error").html('重复密码错误,请再次设置');
				} else {
					stpswd(password.join(""));
				}
			}
	...
	function stpswd(password) {
		$("#error").html('');
		$.ios.ajax({
			url : '${ctx}/wap/user/stpswd?openid=${openid}',
			msg : { 
				text : '正在设置', succ : '设置成功',
				fail : '设置失败', warn : '系统繁忙'
			},
			data : { 'touchsPassword' : password },
