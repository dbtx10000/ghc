...
$.ios = {
	...
	ajax : function(opts) {
		...
		var _opts = $.extend({
			timeout : 3e4, async : false,
			type : 'post', dataType : 'json', 
			beforeSend : function() {
				var text = opts.msg.text;
				if (text == null) {
					text = 'Loading';
				}
				overlay = iosOverlay({
			...
			error : function(e) {
				var l_warn_msg = opts.msg.warn;
				if (l_warn_msg == null) {
					l_warn_msg = g_warn_msg;
				}
				window.setTimeout(function() {
					overlay.update({
						icon	: g_fail_ico,
		...
		_opts.success = function(data) {
			if ($.isFunction(opts.success)) {
				var back = opts.success(data);
				alert("back:" + JSON.stringify(back));
				if (back != null) {
					var flag = back.flag;
					if (flag == true || flag == 'true' || flag=='succ') {
						window.setTimeout(function() {
