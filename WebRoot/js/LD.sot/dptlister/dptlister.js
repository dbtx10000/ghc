...
function DPTLister(options) {
	this.options = $.extend({ 
		...
		sortWay : 'create_time desc', colSort : false, scFixed : false,
		txFixed : true, btFixed : 100, defined : false
	}, options);
	this.init = function() {
	...
	this.list = function(is_not_refresh) {
		var self = this, $root = $("#" + self.options.ID);
		/*************************************/
		var pager = 1;
		var store = new LeopardStorage('dptlister.page');
		if (is_not_refresh) { 
			var pager = store.get(location.href + '#' + self.options.ID);
			if (pager != null) {
				pager = parseInt(pager.value);
			}
		}
		store.set(location.href, new String(pager));
		$root.find("#ld_dptl_page_num").val(pager); 
		/*************************************/
...
var dptlso = {
	setPageInfo : function(dptl, data) {
		...
		curRecord = curRecord ? totalCount % pageSize : pageSize;
	...
	goTargetPage : function(dptl, btn) {
		...
		var store = new LeopardStorage('dptlister.page');
		store.set(location.href + '#' + dptl.options.ID, new String(pageNo));
		$root.find('#ld_dptl_page_num').val(pageNo); 
		dptl.list(true);
	}
};